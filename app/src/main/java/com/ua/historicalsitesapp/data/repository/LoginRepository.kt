...
class LoginRepository(
    ...
    private suspend fun storeAuthTokens(tokens: LoggedInUser) {
        ...
        dataStore.edit {
            it[datastoreAccessTokenKey] = encryptedAccessToken.let { encrypted ->
                Base64.encodeToString(encrypted, Base64.DEFAULT)
            }
            it[datastoreRefreshTokenKey] = encryptedRefreshToken.let { encrypted ->
                Base64.encodeToString(encrypted, Base64.DEFAULT)
            }
        }
    }
    ...
    private fun encrypt(data: String, forProp: String): ByteArray? {
        val cipher = Cipher.getInstance("AES/GCM/NoPadding")
        cipher.init(Cipher.ENCRYPT_MODE, getSecretKey())
        runBlocking {
            storeInitBuffer(cipher.iv, forProp)
        }
        return cipher.doFinal(data.toByteArray())
    }

    private fun decrypt(encrypted: ByteArray, forProp: String): String {
        val cipher = Cipher.getInstance("AES/GCM/NoPadding")
        val iv = runBlocking {
            getInitBuffer(forProp)
        }
